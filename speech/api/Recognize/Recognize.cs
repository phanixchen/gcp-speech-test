...
namespace GoogleCloudSamples
{
    ...
    public class Recognize
    {
        ...
        static object SyncRecognize(string filePath)
        {
            ...
            foreach (var result in response.Results)
            {
                foreach (var alternative in result.Alternatives)
                {
                    ...
                    Console.WriteLine(pc.ConvertToCommand(alternative.Transcript));
                }
            }
            return 0;
        }
        ...
        static NAudio.Wave.WaveInEvent waveIn = new NAudio.Wave.WaveInEvent();
        static SpeechClient speech = SpeechClient.Create();
        static SpeechClient.StreamingRecognizeStream streamingCall = speech.StreamingRecognize();
        ...
        static async Task<object> StreamingMicRecognizeAsync(int seconds)
        {
            ...
            //var streamingCall = speech.StreamingRecognize();
            // Write the initial request with the config.
            Console.WriteLine("Init Microphone");
            await streamingCall.WriteAsync(
                new StreamingRecognizeRequest()
                {
                    StreamingConfig = new StreamingRecognitionConfig()
            ...
            Task printResponses = Task.Run(async () =>
            {
                ...
                while (await streamingCall.ResponseStream.MoveNext(
                    default(CancellationToken)))
                {
                    //Console.WriteLine(streamingCall.ResponseStream.Current.Results.Count);
                    foreach (var result in streamingCall.ResponseStream
                        .Current.Results)
                    {
                        //Console.WriteLine(result.Alternatives.Count);
                        ...
                        foreach (var alternative in result.Alternatives)
                        {
                            ...
                            if (lastString.Length <= alternative.Transcript.Length)
                            {
                                ...
                                arrCallFunction = stc.TransToCallFunction(lastString);
            ...
            Console.Write(DateTime.Now.ToLongTimeString());
            Console.WriteLine(", Function Call(ending): " + string.Join(" -=- ", arrCallFunction));
            lastString = "";
